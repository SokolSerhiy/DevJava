...
public class AmountController {
	...
	@GetMapping
	public String show(Model model, @PageableDefault(direction=Sort.Direction.DESC, sort="id") Pageable pageable, @ModelAttribute("filter") AmountFilter filter){
	...
	public String delete(@PathVariable Long id, @PageableDefault(direction=Sort.Direction.DESC, sort="id") Pageable pageable, @ModelAttribute("filter") AmountFilter filter){
		amountService.delete(id);
		return "redirect:/admin/amount"+getParams(pageable, filter);
	}
	
	@GetMapping("/update/{id}")
	public String update(@PathVariable Long id, Model model, @PageableDefault(direction=Sort.Direction.DESC, sort="id") Pageable pageable, @ModelAttribute("filter") AmountFilter filter){
		model.addAttribute("amount", amountService.findForm(id));
		return show(model, pageable, filter);
	}
	
	@PostMapping
	public String save(@ModelAttribute("amount") @Valid AmountForm amount,BindingResult br, Model model, SessionStatus status, @PageableDefault(direction=Sort.Direction.DESC, sort="id") Pageable pageable, @ModelAttribute("filter") AmountFilter filter){
	...
	private String getParams(Pageable pageable, AmountFilter filter){
		...
		if(!filter.getIngredientId().isEmpty()){
			for (Long id : filter.getIngredientId()) {
				builder.append("&ingredientId=");
				builder.append(id);
			}
		}
		if(!filter.getSystemId().isEmpty()){
			for (Long id : filter.getSystemId()) {
